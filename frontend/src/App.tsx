...
import { Alert, Avatar, Badge, Button, Card, Checkbox, Col, Empty, Flex, Input, Layout, List, Menu, Modal, Pagination, Popconfirm, Popover, Radio, Row, Select, Skeleton, Space, Statistic, Tag, Tabs, Typography, Upload, message } from 'antd'
import { AppstoreOutlined, BookOutlined, CheckOutlined, CloudUploadOutlined, DownloadOutlined, FilePdfOutlined, FileTextOutlined, LockOutlined, LogoutOutlined, PlusOutlined, ReloadOutlined, RobotOutlined, SearchOutlined, SettingOutlined, ThunderboltFilled } from '@ant-design/icons'
...
function SidebarAiTools({ isPlus }: { isPlus: boolean }) {
  ...
  return <div className="sidebar-ai-tools"><div className="sider-label">AI STUDY TOOLS</div>{SIDEBAR_TOOLS.map(tool => { const locked = Boolean(tool.plusOnly && !isPlus); const selectedDocument = documents.find(document => document.id === selected[tool.key]); return <Popover key={tool.key} trigger="click" placement="rightTop" open={openTool === tool.key} onOpenChange={open => setOpenTool(open ? tool.key : undefined)} content={picker(tool)}><button type="button" className={`sidebar-tool-item ${openTool === tool.key ? 'is-open' : ''}`}><span className={`sidebar-tool-icon ${locked ? 'is-locked' : ''}`}>{locked ? <LockOutlined /> : tool.icon}</span><span className="sidebar-tool-copy"><b>{tool.label}</b><small>{selectedDocument ? selectedDocument.originalFileName : tool.description}</small></span>{locked && <Tag className="sidebar-plus-tag">PLUS</Tag>}<PlusOutlined className="sidebar-tool-plus" /></button></Popover> })}</div>
}
...
function DocumentDetailPage() {
  const { id = '' } = useParams(); const location = useLocation(); const queryClient = useQueryClient(); const active = new URLSearchParams(location.search).get('tab') ?? 'summary'; const [chatInput, setChatInput] = useState(''); const [session, setSession] = useState<ChatSession | null>(null)
  ...
  const activePanel = active === 'mindmap' ? <MindMapPanel data={mindmap} onGenerate={() => generate.mutate('mindmap')} /> : active === 'flashcards' ? <FlashcardsPanel data={flashcards} onGenerate={() => generate.mutate('flashcards')} /> : active === 'quiz' ? <QuizPanel data={quiz} onGenerate={() => generate.mutate('quiz')} /> : active === 'chat' ? <ChatPanel messages={chatQuery.data ?? []} value={chatInput} onChange={setChatInput} onSend={() => void sendChat()} /> : <SummaryPanel summary={summary} loading={summaryLoading} onGenerate={() => generate.mutate('summary')} />
...
function SummaryPanel({ summary, loading, onGenerate }: { summary?: Summary; loading: boolean; onGenerate: () => void }) { if (loading) return <Skeleton active />; if (!summary) return <EmptyAi title="Chưa có bản tóm tắt" description="AI sẽ chắt lọc các ý chính, định nghĩa và checklist ôn tập cho bạn." action={onGenerate} />; return <div className="summary-content"><div className="markdown-lite">{summary.content.split('\n').map((line, index) => <p key={index}>{line || <>&nbsp;</>}</p>)}</div><Button onClick={onGenerate}>Tạo lại</Button></div> }
